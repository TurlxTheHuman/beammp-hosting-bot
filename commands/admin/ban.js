...
module.exports = {
  name: "banuser",
  aliases: ["Banuser", "databaseban", "userban"],
  category: "admin",
  run: async (bot, message, args) => {
    ...
    // TESTING
    if (users[message.author.id].isAdmin === "false" && users[message.author.id].isReseller === "true" && users[mention.user.id].fromWho !== message.author.username) {
      return message.reply("You cannot ban that user, you did not give that user the plan!\n Do `.requestban [user] [reason]` To request a ban!");
    }
    ...
    if (users[mention.user.id].isBanned === "true") {
      return message.reply("User Is Already Banned!");
    }


    if (!mention) {
      return message.reply("Usage: .banuser [user] [reason]");
    }
    ...
    users[person].isBanned = "true";
    users[person].banReason = banReason;
    users[person].bannedBy = message.author.id
    ...
    logembed = new discord.MessageEmbed()
      ...
      .setDescription(`<@${person}>\n Has Been Banned From The ${config.name} Database.`)
